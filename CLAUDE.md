...
- **Composite tools (`get_job_status` / `send_job` / `setup_cut`)**: additive orchestrators built on top of the primitives. `send_job` finds the output module by button-label match (`send file`/`get device`/`waiting for file`), detects an on/off gate as the *immediate* upstream neighbor only (not multi-hop), checks `USBDevice.opened` (not just granted-list length) to decide whether to click Get Device, and requires the lower toggle to read "waiting for file" post-click as the success contract — anything else returns `status: "failed"` with `at_step` and `reason`. Keep every low-level tool intact — the composites are for the 80% path; primitives remain the escape hatch.
