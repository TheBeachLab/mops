...
| `set_parameters` | Set multiple parameters across modules in one call |
...
| `set_physical_size` | Set output size in mm/cm/in for PNG files — calculates and sets correct DPI automatically |
...
- **Event dispatch**: Mods modules are inconsistent — some listen for `'input'` events (e.g., PNG reader DPI), some for `'change'` (e.g., threshold), some for both. `setModuleInput` dispatches both `'input'` and `'change'` with `{ bubbles: true }` to cover all patterns. Label matching is case-insensitive.
- **`<select>` elements**: `get_program_state` includes `<select>` dropdowns with their available `options`. `set_parameter` handles them alongside `<input>` elements.
- **PNG dimensions**: Read directly from the file's IHDR chunk (bytes 16-19 = width, bytes 20-23 = height, big-endian uint32). Never rely on DOM text scraping or moduleOutput events for pixel dimensions.
- **`set_physical_size` module targeting**: The DPI parameter exists on the reader module (e.g., "read PNG"), but other modules (e.g., "distance transform") may also have "dpi" in parameter labels. Always search by reader module name first (`read`, `png`, `svg`, `image`), not by first DPI match across all modules.
- **Processing signals**: File loading and button actions wait for completion using `moduleOutput` events (new mods) or `'ready'` postMessage (old mods/modsproject.org), with a timeout fallback. Avoid fixed `waitForTimeout` calls.
...
- **Granted ≠ acquired**: `navigator.usb.getDevices()` returns persistently *granted* devices (permission state, kept by the persistent profile across sessions). It does **not** mean mods has actively `device.open()`-ed the handle for the current page lifetime. Use `USBDevice.opened` (W3C-spec readonly boolean) as the canonical probe for active acquisition. `getGrantedDevices()` exposes this as `opened: true|false` on each entry. Length-of-list checks are misleading after the first session because grants survive forever.
...
- **WebUSB output module button layout**: three buttons. Top row has `Get Device` and `Forget` (static labels — never change). Below is a single toggle that cycles `waiting for file` (no toolpath) → `send file` (toolpath ready) → click → `waiting for file` (toolpath consumed = success). Identify the toggle by regex `/send file|waiting for file/i`, **never** by `buttons[0]` (which is the static "Get Device"). Post-send: `waiting for file` means the toolpath was transmitted successfully; still-`send file` means the click was a silent no-op (no device acquired).
- **Composite tools (`get_job_status` / `send_job` / `setup_cut`)**: additive orchestrators built on top of the primitives. They are **idempotent and state-convergent**: callable from any post-setup state, they walk back through preconditions and auto-fix what's missing. `send_job` reads the lower toggle to decide whether to calculate (`send file` → skip; `waiting for file` or missing → calculate), checks `USBDevice.opened` to decide whether to click Get Device, ensures the gate is on, then sends and **polls** the toggle (not a fixed sleep) for the flip to "waiting for file" as the success contract. The toggle flip IS the "data went through the cable" signal — fixed sleeps caused false negatives where the cutter actually started cutting but mods's UI hadn't updated when we read it. For non-recoverable preconditions (no program/file, no calculator module), responses include `next_action` telling the caller exactly which primitive to invoke. `skip_calculate` parameter is deprecated (accepted but ignored) — calculation is now state-driven. Keep every low-level tool intact — the composites are for the 80% path; primitives remain the escape hatch.
